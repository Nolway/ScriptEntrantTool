...
class Opening{
	...
	*/
	public function __construct(array $infos){
		$this->infos = $infos;
		$this->checkInfos();
		$this->dao = new DAO($this->serveur_id, $this->campagne_id + 1000);
		if($this->getProspectStatus() == 0){
			$this->getProspect();
		}else{
			$this->createNewProspect();
		}
		$this->getAgent();
		$this->getPseudo();
    ...
    */
	...
	/**
	 * Récupère le status du prospect (0 = normal ; 1 = nouveau prospect ; // 2 = hors campagne)
	 *
	 * @return int Valeur du status
	 */
	public function getProspectStatus(){
		switch($this->prospect_status){
			case 'h':
				return 2;
			break;
			case 'n':
	...
	/**
	 * Création d'un nouveau prospect dans la table Prospect
	 */
	private function createNewProspect(){
		try{
			$req_refprospect = $this->dao->getCamp()->query("SELECT Max(Refprospect) as maxref FROM Prospect;");
			$this->prospect_id = $req_refprospect->fetch(PDO::FETCH_ASSOC)['maxref'] + 1;
		}catch(PDOException $e){
			// Vérifie si le résultat à bien été sauvgardé
			$trace = debug_backtrace();
			trigger_error("Erreur dans la création d'une nouvelle refprospect dans ".$trace[0]['file']."
					à la ligne ".$trace[0]['line'].". Erreur PDO : ".$e->getMessage().".", E_USER_NOTICE);
			exit();
		}

		try{
			$req_createProspect = $this->dao->getCamp()->prepare("INSERT INTO Prospect (Refprospect) VALUES (:Refprospect);");
			$req_createProspect->execute(array(
				"Refprospect" => $this->prospect_id
			));
		}catch(PDOException $e){
			// Vérifie si le résultat à bien été sauvgardé
			$trace = debug_backtrace();
			trigger_error("Erreur dans la création du prospect dans ".$trace[0]['file']."
					à la ligne ".$trace[0]['line'].". Erreur PDO : ".$e->getMessage().".", E_USER_NOTICE);
			exit();
		}
	}
